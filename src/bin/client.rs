...
async fn main() {
    ...
    // GameManager 쓰레드
    let mgr_handle = spawn_manager(socket, mgr_writer, game_reader);

    // Game 쓰레드
    let game_handle = spawn_game(game_writer, mgr_reader);
...
// 게임 쓰레드
fn spawn_game(
    game_writer: Sender<Message>,
    mut mgr_reader: Receiver<Message>,
) -> tokio::task::JoinHandle<()> {
    return tokio::spawn(async move {
        // run game
        initscr();
        ...
        // Teardown
        game_writer.send(Message::GameOver).await.unwrap();
